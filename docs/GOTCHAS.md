...
- 名言首次逐字入场需同时检查外层 opacity 与加载态选择器优先级：`.quote-slide.is-active .quote-char` 曾覆盖较弱的加载规则，配合 stage 整块渐入表现为只有淡入。缓存字体还需留出初始姿态绘制帧；回归脚本 `apps/site/scripts/check-quote-entry.py` 对首次、换句、缓存刷新采样中间态。
