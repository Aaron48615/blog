...
- [~] Site 首页倒三角 scroll-away 生产构建修复 | owner: zcode (GLM 5.3 Flash) | 范围: 仅 `apps/site/src/components/Hero.astro` 的 `.scroll-cue` 倒三角淡出块；不改名言轮播、Hero 入场动画、布局或响应式 | 结果: 待实现 | 验收: `pnpm --filter site build` 通过；build 产物 `dist/_astro/*.css` 中滚动动画声明必须存活，且不能被压缩器合并进 `animation` 简写；按实际滚动位置采样验证倒三角随首屏退出而上移淡出；dev 模式与生产构建表现一致；减少动态效果/无 JS 时保留静态可读；名言轮播与 Hero 入场动画不受影响 | 边界: 不扩大至未点名的动画/组件；真机 Safari/Firefox 待补验 | 分支: `feature/site-triangle-scrollaway-fix`（待 zcode 实施）
