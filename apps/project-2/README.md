...
所有视口统一按 `min(视口宽度, 375px)` 计算 rem；设计稿仍为 750px，`postcss-pxtorem.rootValue` 仍为 75。`#app` 以 `10rem` 为最大宽度居中，桌面实际为 375px，两侧背景按用户最新要求为纯白 `#ffffff`，首页内部底色为 `#f7f4f2`；375px 及以下全宽。高度随视口和内容增长，不锁定 667px。
