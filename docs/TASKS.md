...
- [~] Site projects 页补充 project-2 在线预览链接 | owner: Codex（用户指定） | 范围: 仅 `apps/site/src/pages/projects.astro` 中拾光集移动商城卡片的 `previewLink`；不改卡片样式、布局、首页 FeaturedProjects 或其他页面 | 结果: 待实现 | 验收: `pnpm --filter site build` 通过；`/projects` 页面拾光集卡片出现"在线预览"按钮且指向 project-2 生产域名；点击后正常跳转；project-1 卡片保持双按钮不变；其他三页回归 200 | 分支: `feature/site-project2-preview-link`（待 Codex 实施）
