...
> project-2：图片代理与首页加载修复已完成；25 项测试、build、假密钥产物检查通过。`921a205` 的新预览 `project-2-c0me2ahv2-aaronsblog.vercel.app` 已在线验证三条 SPA 路由、三个首页接口和 15 张真实图片，用户复验反馈“现在能加载出来了”。代码与预览可交 Claude Code 审查，但不代表下单、支付等业务全量验收。Production 环境变量尚未设置，真实 `VITE_AI_API_KEY` 与密钥安全验收冲突，待确认本轮 AI 先保留兜底、服务端共享 AI 代理另做。
...
- [~] 旧项目 2 API 代理与 Vercel 部署（第二段） | owner: Codex | 进度: 商城/图片代理、SPA 回退、首页失败隔离与重试已推送；25 项测试、build、假密钥产物检查通过，新预览三条 SPA 路由、三个首页接口、15 张图片在线检查通过，用户已确认页面恢复加载；代码待 Claude Code 审查，Production/AI 配置仍待确认，整体任务暂不标完成 | 验收: Vercel 预览首页与深层路由可访问、图片正常，第三方 API 调用正常，无真实 AI 密钥打入前端产物 | 分支: `feature/migrate-project-2-deploy`
