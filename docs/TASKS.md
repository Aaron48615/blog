...
1. apps/site 框架：暂定 Astro，待用户确认或改 Next.js
2. 旧项目源目录：待用户告知（候选：~/mobile-shop、~/project、~/GameHub）
3. 首次提交：骨架尚未 commit，待用户授权

> 确认后立即执行 Phase 1 任务 1（Astro 脚手架），验收标准见下。

## Phase 1 — 脚手架与部署管线
- [ ] apps/site Astro 脚手架 + 基础布局（首页/关于/项目列表） | owner: 待认领 | 验收: `pnpm --filter site dev` 可跑，三页可访问
...
## Phase 2 — 旧项目迁移
- [ ] 旧项目 1 迁入 `apps/project-1`（来源目录待确认） | owner: 待认领 | 验收: build 通过，预览环境可访问，第三方 API 调用正常
- [ ] 旧项目 2 迁入 `apps/project-2`（来源目录待确认） | owner: 待认领 | 验收: 同上
