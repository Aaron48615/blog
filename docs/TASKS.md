...
> project-2：用户已确认旧预览 `/api/indexImgs` 正常，但反馈页面缺少轮播图、商品图。已修复图片域名 HTTPS 证书问题与首页接口失败牵连；25 项测试、build、假密钥产物检查通过。`921a205` 的新预览 `project-2-c0me2ahv2-aaronsblog.vercel.app` 已在线验证三条 SPA 路由、三个首页接口和 15 张真实图片，浏览器登录后的实际显示仍待用户复验。Production 环境变量尚未设置，真实 `VITE_AI_API_KEY` 与密钥安全验收冲突，待确认本轮 AI 先保留兜底、服务端共享 AI 代理另做。
...
- [~] 旧项目 2 API 代理与 Vercel 部署（第二段） | owner: Codex | 进度: 固定来源 `/shop-images` 图片代理及全局地址转换、首页失败隔离与重试已推送（`921a205`）；25 项测试、build、假密钥产物检查通过，新预览在线检查三条 SPA 路由、三个首页接口、15 张图片通过；登录后页面目检与 Production/AI 配置仍待确认 | 验收: Vercel 预览首页与深层路由可访问、图片正常，第三方 API 调用正常，无真实 AI 密钥打入前端产物 | 分支: `feature/migrate-project-2-deploy`
